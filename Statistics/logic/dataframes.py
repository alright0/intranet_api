...
class up_puco_table:
                    ...
                    for line_side in IBEA_CAMERA_MAP[line]:
                        ...
                        camera_name_string = (
                            "{}: {:.2f}% ({} шт. выброшено из {} шт.)".format(
                                line_side,
                                camera_side_df["pcs_rejected"].sum() / camera_side_df["pcs_total"].sum() * 100,
    ...
    def _parse_camera(self):
        ...
        for line in self.lines:
            if line in IBEA_CAMERA_MAP:
                df_camera_lvl_0 = pd.DataFrame(list(Camera.get_camera_info(self.date_start, self.date_end, line)))
        ...
        if not camera_line_df.empty:
            ...
            camera_line_df["defect_rate"] = camera_line_df["defect_rate"].apply(
                lambda x: self.defrate_cutoff if x > self.defrate_cutoff else x)
    ...
    def cumsum_to_any(final_df_by_line, cumsum_counter):
        ...
        return final_df_by_line[cumsum_counter].diff().fillna(0).clip(lower=0).astype(int)
